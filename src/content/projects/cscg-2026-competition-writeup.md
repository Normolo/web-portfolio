...
One challenge used a SHA-1 secret prefix MAC for authentication. The vulnerability: SHA-1's Merkle Damgård construction means that if you know the hash of a message, you can extend it without knowing the secret. I implemented the full attack from scratch in pure Python, with no external libraries, manually reconstructing the internal hash state and forging a valid authenticated message.
